...
export function App() {
  ...
  // Inicializar selectores cuando llega el contexto.
  useEffect(() => {
    if (ctx && !sucursalId) {
      const s = ctx.sucursales[0];
      if (s) { setSucursalId(s.id); setCajaId(s.cajas[0]?.id ?? ''); }
      setVendedorId(ctx.vendedores[0]?.id ?? '');
    }
  }, [ctx, sucursalId]);
  ...
  const { data: productos = [], isFetching } = useQuery({
    queryKey: ['productos', sucursalId, search],
    queryFn: () => api.productos(sucursalId, search),
    enabled: !!sucursalId,
  });
  ...
  const { data: actividad = [] } = useQuery({
    queryKey: ['actividad', sucursalId], queryFn: () => api.actividad(sucursalId), enabled: !!sucursalId,
  });
  ...
  function agregar(p: ProductoDTO, v: VarianteDTO) {
    ...
    setCart((prev) => {
      ...
      if (ex) {
        if (ex.cantidad >= ex.stock) return prev;
        return prev.map((i) => (i.varianteId === v.id ? { ...i, cantidad: i.cantidad + 1 } : i));
      }
      return [...prev, {
        varianteId: v.id, producto: p.nombre, talle: v.talle, color: v.color,
        precio: v.precio, cantidad: 1, stock: v.stock, requiereAjuste: false,
      }];
    });
  }
    ...
    <div className="min-h-screen bg-stone-100 text-stone-800">
      ...
      <header className="bg-stone-900 text-stone-100 px-5 py-3 flex flex-wrap items-center gap-4 shadow">
        ...
        <div className="flex items-center gap-2 ml-auto text-sm">
          ...
          <Selector label="Caja" value={cajaId} onChange={setCajaId}
            options={sucursal?.cajas.map((c) => ({ v: c.id, t: c.nombre })) ?? []} />
          <Selector label="Vendedor" value={vendedorId} onChange={setVendedorId}
            options={ctx?.vendedores.map((v) => ({ v: v.id, t: v.nombre })) ?? []} />
        </div>
      </header>
              ...
              <div key={i.varianteId} className="border border-stone-200 rounded-lg p-2">
                ...
                <div className="flex items-center gap-2 mt-2">
                  <div className="flex items-center border border-stone-300 rounded-md">
                    ...
                    <button onClick={() => cambiarCantidad(i.varianteId, 1)} disabled={i.cantidad >= i.stock}
                      className="px-2 text-stone-600 hover:bg-stone-100 disabled:text-stone-300">+</button>
                  </div>
                  <label className="text-xs text-stone-500 flex items-center gap-1 ml-auto cursor-pointer">
                    <input type="checkbox" checked={i.requiereAjuste} onChange={() => toggleAjuste(i.varianteId)} />
                    ajuste (ruedo/entalle)
                  </label>
                </div>
              </div>
            ))}
          ...
          <div className="border-t border-stone-200 mt-3 pt-3">
            ...
            <div className="flex items-center gap-2 mb-3">
              ...
              <select value={medio} onChange={(e) => setMedio(e.target.value)}
                className="border border-stone-300 rounded-md px-2 py-1 text-sm flex-1">
                {MEDIOS.map((m) => <option key={m} value={m}>{m}</option>)}
              </select>
            </div>
            <button disabled={cart.length === 0 || confirmar.isPending || !cajaId}
              ...
              {ventas.map((v) => (
                <div key={v.id} className="flex items-center justify-between text-sm border-b border-stone-100 pb-1.5">
                  <div>
                    ...
                    <div className="text-xs text-stone-400">{v.medios.join(', ')}
                      {v.estadoEntrega === 'PENDIENTE_AJUSTE' && <span className="ml-1 text-amber-600">· ajuste</span>}
                    </div>
                  </div>
            ...
            <div className="space-y-1 max-h-[30vh] overflow-y-auto text-xs">
              {actividad.map((a, k) => (
                <div key={k} className="flex items-center gap-2">
                  ...
                  <span className="text-stone-400">{a.tipo === 'evento' ? a.estado : a.estado}</span>
      ...
      {toast && (
        <div className="fixed bottom-5 left-1/2 -translate-x-1/2 bg-stone-900 text-white px-5 py-3 rounded-lg shadow-lg text-sm">
          {toast}
        </div>
      )}
    </div>
  ...
  return (
    <label className="flex items-center gap-1.5">
      ...
      <select value={value} onChange={(e) => onChange(e.target.value)}
        className="bg-stone-800 border border-stone-700 rounded-md px-2 py-1 text-stone-100">
